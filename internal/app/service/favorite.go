...
func AddFavorite(
...
) (*model.Event, someerr.Error) {
	event, err := model.GetEventByRefID(ctx, db, refID)
	switch {
	case errors.Is(err, pgx.ErrNoRows):
		return nil, someerr.NotFound.Error("event not found")
	case err != nil:
	...
	if userID == event.UserID {
		return nil, someerr.PermissionDenied.Error("can't favorite own event")
	}

	// check if favorite already exists
	_, err = model.GetFavoriteByUserEvent(ctx, db, userID, event.ID)
	if err == nil {
		return nil, someerr.AlreadyExists.Error("favorite already exists")
	}
...
func RemoveFavorite(
	...
	switch {
	case errors.Is(err, pgx.ErrNoRows):
		return someerr.NotFound.Error("event not found")
	case err != nil:
	...
	switch {
	case errors.Is(err, pgx.ErrNoRows):
		return someerr.NotFound.Error("favorite not found")
	case err != nil:
		return someerr.Internal.Error("db error")
	}
...
) ([]*model.Event, *Pagination, someerr.Error) {
...
func GetFavoriteEventsCount(
	ctx context.Context, db model.PgxHandle, userID int,
) (*model.BifurcatedRowCounts, someerr.Error) {
