...
func NewAPI(db *model.DB, tpl res.TemplateMap, csrfKey []byte, isProd bool) *API {
	...
	r.Group(func(r chi.Router) {
		...
		r.Get("/events/{eRefId:[0-9a-z]+}/add-item", ah.ShowCreateEventItemForm)
		r.Post("/events/{eRefId:[0-9a-z]+}/add-item", ah.CreateEventItem)
		r.Get("/events/{eRefId:[0-9a-z]+}/items/{iRefId:[0-9a-z]+}/edit", ah.ShowEventItemEditForm)
		r.Post("/events/{eRefId:[0-9a-z]+}/items/{iRefId:[0-9a-z]+}/edit", ah.UpdateEventItem)
