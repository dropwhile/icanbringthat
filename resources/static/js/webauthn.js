...
async function registerUser(extra_headers) {
    var register_api = "/authn/register";
    if (!window.PublicKeyCredential) {
        alert("Sorry, this browser does not support WebAuthn");
        return;
    }
    ...
    try {
        ...
        const challenge = await fetch(register_api, {
            method: 'GET',
            mode: 'same-origin',
            cache: 'no-cache',
        ...
        const finalise = await fetch(register_api, {
            method: 'POST',
            mode: 'same-origin',
            cache: 'no-cache',
            credentials: 'same-origin',
